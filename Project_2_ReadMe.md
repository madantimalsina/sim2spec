...
> **x-axis — sample index:** Each step along the x-axis is one digitized sample from the light detector's readout electronics. It is a plain integer counter (0, 1, 2, …) that increases with time. The spacing between samples is set by the light readout sampling rate, so the x-axis is effectively a relative time axis within one waveform, not an absolute clock time.
...
> **x-axis — sample index:** Same as the waveform QA plot above: each tick is one digitized sample, counting up from 0. Reading left to right is reading forward in time within a single trigger window. A spike appearing near sample 850, for example, means a photon signal arrived about 850 sample intervals after the trigger opened.
...
> **x-axis — ADC value (dimensionless integer):** ADC stands for analog-to-digital converter. It converts a physical charge signal into a number the computer can store. A value of 0 means no charge was measured; a higher number means a stronger signal. The readout here uses a 10-bit range, so values run from 0 to 1023. Most real charge signals appear between roughly 10 and 255 ADC. The tall spike near 0 comes from bookkeeping packets (timing and sync records) that carry no real charge — their ADC reads as zero.
...
> **x-axis — timestamp (clock ticks):** Same unit as the timestamp histogram — raw LArPix clock ticks at 10 MHz. Each vertical cluster of dots in this scatter plot is one neutrino event: all the charge packets from that interaction arrive at roughly the same simulated time, so they stack up at the same x position. The spacing between clusters tells you how far apart the events were in simulated time.
...
> **x-axis — event ID (integer index):** Each bar corresponds to one simulated neutrino interaction, numbered starting from 0. Event ID is just a label assigned in order — it does not represent time or energy. The height of each bar is the number of segments (simulated particle trajectory pieces) recorded for that event. A tall bar means the interaction was complex and produced a lot of detector activity; a short bar means it was a simpler, quieter interaction.
